...
class Storage(gobject.GObject):
    ...
    def __init_db_file(self, database_dir):
        ...
        if not os.path.exists(db_path):
            ...
            old_db_path = os.path.join(xdg_data_home,
            ...
            if os.path.exists(old_db_path):
                ...
                try:
                    ...
                    if os.path.exists(os.path.join(module_dir, "data")):
                        ...
                        # get ./data from ./src/hamster_lite/storage/db.py
                        data_dir = os.path.join(module_dir,
                                                '..', '..', '..', 'data')
                logger.warning("Database not found in %s"
                               " - installing default from %s!"
                               % (db_path, data_dir))
    ...
    def get_tags(self, only_autocomplete=False):
        return self.fetchall(
            "select * from tags %s order by name"
            % "where autocomplete != 'false'" if only_autocomplete else "")
    ...
    def _get_tag_ids(self, tags):
        ...
        set_complete = [str(tag["id"]) for tag in db_tags
                        if tag["autocomplete"] == "false"]
        if set_complete:
            changes = True
            query = "update tags set autocomplete='true' where id in (%s)"
            self.execute(query % ", ".join(set_complete))
    ...
    def update_autocomplete_tags(self, tags):
        ...
        to_uncomplete = [str(tag["id"]) for tag in gone
                         if tag["occurences"] > 0
                         and tag["autocomplete"] != 'false']  # noqa: W503
        if to_delete:
            self.execute(
                "delete from tags where id in (%s)" % ", ".join(to_delete)
            )
            print('ac_tags_delete:', to_delete)
        if to_uncomplete:
            print('ac_tags_uncomplete:', to_uncomplete)
            self.execute(
    ...
    def get_categories(self):
        query = "SELECT id, name FROM categories ORDER BY lower(name)"
    ...
    def update_activity(self, id, name, category_id):
        ...
        query = "select id from facts where activity_id = ?"
        affected_ids = [res[0] for res in self.fetchall(query, (id,))]
    ...
    def change_category(self, id, category_id):
        ...
        query = "select name from activities where id = ?"
        activity = self.fetchone(query, (id,))
        existing_activity = self.get_activity_by_name(activity['name'],
                                                      category_id)
        ...
        if existing_activity:  # ooh, we have something here!
            # first move all facts from the movable activity to the new one
            update = """
                       UPDATE facts
                          SET activity_id = ?
                        WHERE activity_id = ?
    ...
    def _create_facts(self, fact_dicts):
        ...
        if not fact_dicts:
            return fact_dicts  # be it None or whatever

        facts = []
        for fact_id, fact_tags in \
                itertools.groupby(fact_dicts, lambda f: f["id"]):
    ...
    def _squeeze_in(self, start_time):
        ...
        # in the latter case go only few hours ahead. everything else
        # is madness, heh
        query = """
           SELECT a.*, b.name
             FROM facts a
    ...
    def _solve_overlaps(self, start_time, end_time):
        ...
        for row in conflicts:
            ...
            # won't eliminate as it is better to have overlapping entries
            # than loosing data
            if start_time < fact.start_time and end_time > fact_end_time:
                continue

            # split - truncate until beginning of new entry
            #         and create new activity for end
               ...
               fact.start_time < end_time < fact_end_time:
                ...
                tag_update = """INSERT INTO fact_tags(fact_id, tag_id)
                                     SELECT ?, tag_id
                                       FROM fact_tags
                                      WHERE fact_id = ?"""
                self.execute(tag_update, (new_fact_id, fact.id))
    ...
    def add_fact(self, fact):
        ...
        tag_set = set((tag[1] for tag in tags))
        ...
        if abs((hamster_now() - start_time).days) <= 1:
            ...
            if previous and previous.start_time <= start_time:
                # check for the same one, in that case no need to restart
                if previous.activity_id == activity_id \
                   and set(previous.tags) == tag_set \
                   and (previous.description or "") == (fact.description or ''):  # noqa: E501
                    return None
                ...
                if not previous.description \
                   and 60 >= (start_time - previous.start_time).seconds >= 0:
                    ...
                    # one before that is actually same as the one we want to
                    # start (glueing)
                    if len(facts) > 1 and start_time == facts[-2].end_time:
                        before = facts[-2]
                        if before.activity_id == activity_id \
                           and set(before.tags) == tag_set:
                            # resume and return
                            update = """
                                       UPDATE facts
                                          SET end_time = null
        ...
        self.execute([insert] * len(tags), params)
    ...
    def get_facts(self, date, end_date=None, search_terms=""):
        ...
        end = dt.datetime.combine(end_date, split_time) \
    ...
    def remove_activity(self, id):
        """ check if we have any facts with this activity and behave
            accordingly if there are facts - sets activity to deleted = True
            else, just remove it"""
        ...
        if bound_facts > 0:
            self.execute("UPDATE activities SET deleted = 1 WHERE id = ?",
                         (id,))
        else:
    ...
    def _check_index(self, start, end):
        ...
        if rebuild_ids:
            ...
            insert = """INSERT INTO fact_index
                             (id, name, category, description, tag)
                             VALUES (?, ?, ?, ?, ?)"""
            params = [(f.id, f.activity, f.category, f.description,
                       ' '.join(f.tags)) for f in facts]
    ...
    def connection(self):
        if self.con is None:
            self.con = sqlite3.connect(self.db_path,
                detect_types=sqlite3.PARSE_DECLTYPES|sqlite3.PARSE_COLNAMES)  # noqa
    ...
    def run_fixtures(self):
        ...
        if version < current_version:
            ...
            print("updated database from version %d to %d"
